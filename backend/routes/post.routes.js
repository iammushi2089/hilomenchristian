...
// GET /api/posts — Public: all published posts (newest first) 
    ...
    try { 
        const posts = await Post.find({ 
            ...
            .sort({ createdAt: -1 }); res.json(posts);
...
try { 
    ...
    if (!post || post.status === 'removed') return res.status(404).json({ 
        message: 'Post not found' });
        res.json(post);
} catch (err) { res.status(500).json({ message: err.message }); }
});

// POST /api/posts/:id/heart — Member/Admin: toggle heart (like/unlike)
router.post('/:id/heart', protect, async (req, res) => {
    try {
        ...
        const alreadyLiked = post.likedBy.some((uid) => uid.toString() === req.user._id.toString());
        if (alreadyLiked) {
...
// POST /api/posts — Member or Admin: create new post // upload.single('image') handles optional image file
